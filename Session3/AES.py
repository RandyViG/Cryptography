...
def generateKey( ci , table ):
    tableS = [ [0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76 ] ,
    ...
    key = [ ]
    lastColumn = table[ len(table)-1 ]
    #rotation
    aux = lastColumn[0]
    lastColumn = lastColumn[1:]
    lastColumn.append(aux)
    #sbox
    columnS = [ ]
    for element in lastColumn:
        row = ( ( element >> 4 ) & 15 ) 
        column = (element & 15)
        columnS.append( tableS[row][column] )
    #vector Ci
    columnS[0] = columnS[0] ^ ci
    aux = columnS
    for row in table:
        newColumn = [ ]
